...
@section('content')
        ...
        <div class="page-inner mt--5 row justify-content-center">
          <div class="card p-3 col-md-7 row">
            <h2 class="fw-bold">Generate individual auction's report</h2>
            <form id="report_individual" action="{{ route('reports.generate') }}" method="post">
              @csrf
              <input type="text" name="method" value="individual" hidden>
              <div class="row">
                <div class="col-md-12">
                  <div class="form-group">
                    <label for="">Auction :</label>
                    <select name="auction" id="" class="form-control">
                      <option value="" selected disabled>-- Select auction --</option>
                      @foreach ($auctions as $auction)
                          <option value="{{ $auction->id }}">{{ $auction->item->name }} - @currency($auction->best_offer)</option>
                      @endforeach
                    </select>
                  </div>
                </div>
    ...
    @section('js')
        ...
        <script>
          function deleteContentIframe(id) {
        ...
        $('#report_individual').off().on('submit', (e) => {
          e.preventDefault();
          var formData = new FormData($(e.target)[0])
          var route = "{{ route('reports.generate') }}"
          $("#report_individual button[type=submit]").prop('disabled', true)
          ...
          $.ajax({
              ...
              complete: () => {
                  $("#report_individual button[type=submit]").prop('disabled', false)
